/********************************************************
 *  This is the full code for the GNSS device so far 
 *  Last updated: 2024-06-15
 * 
 *  TODO: Split into header files and makemodularize 
 * 
 * What it does so far:
 * - Initializes the display, touch, GNSS, SD card, and sensors
 * - Reads GNSS data from the LC29H module and parses it with TinyGPSPlus
 * - Reads temperature and pressure from the BMP280 sensor
 * - Reads accelerometer and gyroscope data from the QMI8658C sensor
 * - Displays Menus for Home, Live GNSS, Satellites, Signal Quality, FSPL, Campus Survey, and System
 * 
 * Possible future ideas:
 * - Extended Kalman Filter and Sensor Fusion for better GNSS accuracy
 * - Adding Magnetometer for direction tracking when GNSS is unavailable
 * ...
*/
...
/*
  Tenstar ESP32-S3 GNSS Field Tool

  Display:
  - ILI9488-compatible external SPI display
  - XPT2046 touch
  - 320 x 480 portrait
...
// Use 2 because rotation 0 made the screen upside down for you.
#define DISPLAY_ROTATION 2

// Dynamic display updates only every 1.5 seconds to reduce jitter.
...
/*
  Your real physical bottom-button readings were like:
  raw: 271,45
  raw: 53,37
  raw: 148,39
  raw: 244,68

  X was already correct.
  Y was upside down.
  ...
  rawY 45 -> mappedY 434
  rawY 68 -> mappedY 411
  These are inside the displayed bottom nav bar.
*/
...
void setup()
{
  ...
  Serial.println();
  Serial.println("Starting Tenstar GNSS Field Tool");
  ...
  Serial.println("Serial controls: n = next, p = previous, h = home, 1-6 = direct page");
}
...
void createLogHeaderIfNeeded()
{
  ...
  {
    file.println("millis,utc_date,utc_time,fix,latitude,longitude,altitude_m,speed_kmh,satellites,hdop,quality,bmp_temp_c,bmp_pressure_hpa,acc_x,acc_y,acc_z,acc_mag,nmea_sentences");
  }
...
void drawCampusSurveyMenu()
{
  ...
  bigTft.setTextSize(1);
  bigTft.setTextColor(TFT_CYAN, TFT_BLACK);
  bigTft.setCursor(20, 320);
  bigTft.print("CSV logs every 5 seconds.");

  bigTft.setCursor(20, 340);
  bigTft.print("SD: SCK36 MOSI35 MISO37 CS9");
...
void updateCampusSurveyValues()
{
  bigTft.fillRect(100, 65, 210, 230, TFT_BLACK);
  ...
  bigTft.print(LOG_INTERVAL_MS / 1000);
  bigTft.print(" sec");
}
...
void updateSystemValues()
{
  ...
  bigTft.print("Touch map: x=rawX y=479-rawY");

  bigTft.setCursor(20, 370);
  bigTft.print("Buzzer GPIO5");
}
